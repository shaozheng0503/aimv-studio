...
gen() {
  ...
  if jq -e '.data[0].b64_json' "/tmp/gen_${name}.json" > /dev/null 2>&1; then
    jq -r '.data[0].b64_json' "/tmp/gen_${name}.json" | base64 --decode > "$OUT_DIR/${name}.png"
    echo "  OK -> $OUT_DIR/${name}.png"
  else
    echo "  FAIL: $(cat /tmp/gen_${name}.json | head -c 300)"
  fi
}

# 第 1 组：国风古典（4 帧，同一少女主角，叙事递进）

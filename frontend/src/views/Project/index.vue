        ...
        <div class="project-info">
          ...
          <div class="project-title-row">
            <input
              ...
              autofocus
            />
            <h3 v-else @dblclick.stop="startEdit(p, $event)" :title="'双击重命名'">{{ p.title }}</h3>
          </div>
          <div class="project-meta">
            <span :class="['badge', p.status === 'done' ? 'badge-success' : p.status === 'failed' ? 'badge-error' : 'badge-info']">
              {{ statusMap[p.status] || p.status }}
            </span>
            <span class="meta-date">{{ new Date(p.created_at).toLocaleDateString('zh-CN') }}</span>
          </div>
          <div class="project-actions">
            <button class="btn-ghost btn-xs" @click.stop="router.push(`/create/${p.id}`)">进入创作</button>
            <button v-if="p.status === 'done'" class="btn-ghost btn-xs" @click.stop="router.push(`/editor/${p.id}`)">导出</button>
            <button class="btn-ghost btn-xs btn-danger" @click="deleteProject(p, $event)">删除</button>
          </div>
        </div>
      </div>
    </div>
